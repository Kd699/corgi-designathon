/* The motif engine, copied from the teammate app at motif+signal_simulation/ (commit
 * 8625713, src/engine/buttonSpec.ts). Verbatim except for the import path.
 *
 * Copied rather than imported because motif+signal_simulation/ is a separate app outside
 * this Vite root, and Vite's fs.strict refuses to serve across that boundary. If the motif
 * app's spec changes, re-copy this file; nothing here should be edited in place.
 *
 * It takes the same (signals, valence, arousal) our derive() already produces, so the
 * character on the week screen is driven by the same read as the sky behind it.
 */
...
export function deriveMotif(s: Signals, valence: number, arousal: number): ButtonSpec {
