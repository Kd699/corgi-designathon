...
export function useSimWatch(target: number, patch: Patch, enabled = true) {
  useEffect(() => {
    if (!enabled) return;
    let hr = target;
...
export function useMotion(patch: Patch, enabled = true) {
...
export function useClockAndIdle(patch: Patch, enabled = true) {
  useEffect(() => {
    if (!enabled) return;
